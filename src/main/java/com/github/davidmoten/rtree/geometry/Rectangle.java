...
public final class Rectangle implements Geometry, HasGeometry {
    ...
    private Rectangle(float x1, float y1, float x2, float y2, int id) {
        Preconditions.checkArgument(x2 >= x1);
        Preconditions.checkArgument(y2 >= y1);
        this.x1 = x1;
        this.y1 = y1;
    ...
    static Rectangle create(float x1, float y1, float x2, float y2, int id) {
        return new Rectangle(x1, y1, x2, y2, id);
    }
